...
/**
 * A {@link android.view.ViewGroup} that shows one child at a time, allowing the
 * user to swipe horizontally to page between other child views. Based on
 * <code>Workspace.java</code> in the <code>Launcher.git</code> AOSP project.
 * 
 * An improved version of this UI widget named 'ViewPager' is now available in
 * the <a
 * href="http://developer.android.com/sdk/compatibility-library.html">Android
 * Compatibility Package, r3</a>.
 */
public class Workspace extends ViewGroup {
     ...
     * the next screen.
     */
    private static final int MIN_LENGTH_FOR_FLING = 500;
    ...
    private static final int DEFAULT_SCROLL_MODE= 0x00;
    ...
    public interface OnScrollListener {
        void onScroll(float screenFraction, int mode);
    }
    ...
    void handleScreenChangeCompletion(int currentScreen, int mode) {
        ...
        notifyScreenChangeListener(mCurrentScreen, true, mode);
    }

    void notifyScreenChangeListener(int whichScreen, boolean changeComplete, int mode) {
        ...
        if (mOnScrollListener != null) {
            mOnScrollListener.onScroll(getCurrentScreenFraction(), mode);
        }
    }
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (mOnScrollListener != null) {
                mOnScrollListener.onScroll(getCurrentScreenFraction(), mScrollMode);
            }
            postInvalidate();
        } else if (mNextScreen != INVALID_SCREEN) {
            // The scroller has finished.
            handleScreenChangeCompletion(Math.max(0,
                    Math.min(mNextScreen, getScreenCount() - 1)), mScrollMode);
    ...
    public boolean requestChildRectangleOnScreen(View child, Rect rectangle,
        ...
        if (screen != mCurrentScreen || !mScroller.isFinished()) {
            snapToScreen(screen, mScrollMode);
            return true;
        }
        return false;
    }
    ...
    public boolean dispatchUnhandledMove(View focused, int direction) {
        if (direction == View.FOCUS_LEFT) {
            if (getCurrentScreen() > 0) {
                snapToScreen(getCurrentScreen() - 1, mScrollMode);
                return true;
            }
                    ...
                    scrollTo(
                            Math.max(
                                    ...
                                    Math.min(maxScrollX, (int) (mDownScrollX
                                            + mDownMotionX - x))), 0);
                    if (mOnScrollListener != null) {
                        mOnScrollListener.onScroll(getCurrentScreenFraction(), DEFAULT_SCROLL_MODE);
                    }
                ...
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    if (isFling && velocityX > SNAP_VELOCITY && mCurrentScreen > 0) {
                        ...
                        final int bound = scrolledPos <= whichScreen ? mCurrentScreen - 1
                                : mCurrentScreen;
                        snapToScreen(Math.min(whichScreen, bound), DEFAULT_SCROLL_MODE);
                        ...
                        final int bound = scrolledPos >= whichScreen ? mCurrentScreen + 1
                                : mCurrentScreen;
                        snapToScreen(Math.max(whichScreen, bound), DEFAULT_SCROLL_MODE);
                    } else {
                        snapToDestination(DEFAULT_SCROLL_MODE);
                    }
                } else {
                    performClick();
                }
    ...
    void snapToDestination(int mode) {
        final int screenWidth = getScrollWidth();
        final int whichScreen = (getScrollX() + (screenWidth / 2))
                / screenWidth;

        snapToScreen(whichScreen, mode);
    }

    void snapToScreen(int whichScreen, int mode) {
        snapToScreen(whichScreen, false, true, mode);
    }

    void snapToScreen(int whichScreen, boolean fast, boolean notify, int mode) {
        mScrollMode = mode;
        ...
        if (screenChanging && notify) {
            notifyScreenChangeListener(mNextScreen, false, mode);
        }
        invalidate();
    }
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        if (savedState.currentScreen != -1) {
            snapToScreen(savedState.currentScreen, true, true, mScrollMode);
        }
    }
     ...
     */
    public void setCurrentScreen(int screenIndex, int mode) {
        snapToScreen(Math.max(0, Math.min(getScreenCount() - 1, screenIndex)), mode);
    }
     ...
     * @param screenIndex
     */
    public void setCurrentScreenNow(int screenIndex, int mode) {
        setCurrentScreenNow(screenIndex, true, mode);
    }

    public void setCurrentScreenNow(int screenIndex, boolean notify, int mode) {
     ...
     */
    public void scrollLeft(int mode) {
        if (mLocked) {
            return;
        }
        if (mScroller.isFinished()) {
            if (mCurrentScreen > 0) {
                snapToScreen(mCurrentScreen - 1, mode);
            }
        } else {
            if (mNextScreen > 0) {
                snapToScreen(mNextScreen - 1, mode);
            }
        }
    }
     ...
     * exists. This method is a no-op if the Workspace is currently locked.
     */
    public void scrollRight(int mode) {
        if (mLocked) {
            return;
        }
        if (mScroller.isFinished()) {
